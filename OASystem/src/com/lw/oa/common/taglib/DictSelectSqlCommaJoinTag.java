 ...
 **@author yuliang
 */
public class DictSelectSqlCommaJoinTag extends TagSupport implements ConstantUtil {
	/**
	 * 
	 */
	...
	public DictSelectSqlCommaJoinTag() {
	...
	public int doEndTag() throws JspException {
		...
		//list中的busidictid转化为,分割的字符串
		StringBuffer strb = new StringBuffer();
		//select1,属性nullLabel不为空时，option对应的value值做,分割处理
		for (DictEntity command : list) {
			strb.append(SYMBOL_SINGLEQUOTES).append(command.getBusidictid()).append(SYMBOL_SINGLEQUOTES).append(SEPERATORS_COMMA);
		}
		if(!StringUtils.EMPTY.endsWith(strb.toString())){
		...
		if (!StringUtils.isEmpty(this.getNullLabel())) {
			sb.append("<option value=\""+strb+"\">"+this.getNullLabel()+"</option>");
		}
