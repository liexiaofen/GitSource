 ...
 ** @author yuliang
 */
@Service("pd003Service")
public class PD003ServiceImpl implements IPD003Service,ConstantUtil {

	private IMybatisDAO mybatisDAOImpl;

	public PD003ServiceImpl() {
	...
	public List<?> pd003001search(ApplySearchCommand searchCommand) {
		...
		List<ApplyResultCommand> list = (List<ApplyResultCommand>) mybatisDAOImpl
				.queryByObj("pd.pd003.pd003001searchListByPage", searchCommand);
	...
	public int pd003003update(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			flag = mybatisDAOImpl.update( "pd.pd003.pd003003Update", entity);			
			// 获取履历id
	...
	public int pd003003delete(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			// 设置共通更新字段
			DataUtil.setUpdateCol(entity, bean);			
			flag = mybatisDAOImpl.update("pd.pd003.pd003003delete", entity);
			mybatisDAOImpl.commit();
		} catch (Exception e) {
			mybatisDAOImpl.rollback();
			flag = 0;
