 ...
 ** @author yuliang
 */
@Service("pd004Service")
public class PD004ServiceImpl implements IPD004Service,ConstantUtil {

	private IMybatisDAO mybatisDAOImpl;

	public PD004ServiceImpl() {
	...
	public List<?> pd004001search(ApplySearchCommand searchCommand) {
		...
		List<ApplyResultCommand> list = (List<ApplyResultCommand>) mybatisDAOImpl
				.queryByObj("pd.pd004.pd004001searchListByPage", searchCommand);
	...
	public int pd004003update(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			flag = mybatisDAOImpl.update( "pd.pd004.pd004003Update", entity);			
			// 获取履历id
	...
	public int pd004003delete(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			// 设置共通更新字段
			DataUtil.setUpdateCol(entity, bean);			
			flag = mybatisDAOImpl.update("pd.pd004.pd004003delete", entity);
			mybatisDAOImpl.commit();
		} catch (Exception e) {
			mybatisDAOImpl.rollback();
			flag = 0;
