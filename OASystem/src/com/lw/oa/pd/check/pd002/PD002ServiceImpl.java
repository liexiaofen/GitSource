 ...
 ** @author yuliang
 */
@Service("pd002Service")
public class PD002ServiceImpl implements IPD002Service,ConstantUtil {

	private IMybatisDAO mybatisDAOImpl;

	public PD002ServiceImpl() {
	...
	public List<?> pd002001search(ApplySearchCommand searchCommand) {
		...
		List<ApplyResultCommand> list = (List<ApplyResultCommand>) mybatisDAOImpl
				.queryByObj("pd.pd002.pd002001searchListByPage", searchCommand);
	...
	public int pd002003update(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			flag = mybatisDAOImpl.update( "pd.pd002.pd002003Update", entity);			
			// 获取履历id
	...
	public int pd002003delete(ApplyFormCommand command, HttpServletRequest request) {
		...
		try {
			...
			// 设置共通更新字段
			DataUtil.setUpdateCol(entity, bean);			
			flag = mybatisDAOImpl.update("pd.pd002.pd002003delete", entity);
			mybatisDAOImpl.commit();
		} catch (Exception e) {
			mybatisDAOImpl.rollback();
			flag = 0;
