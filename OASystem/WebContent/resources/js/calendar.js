//----------------------------------------------------------------------------
		...
		//add by liudeyuan 20081021
		var _OCX_HEIGHT=247;
		var _OCX_WIDTH=186;
		
		//==================================================== 扩展功能 =======================================================
		// 添加时间 2011-12-28 16:19:00
		// 添加者 周权
		// 用途 选择日期后回调函数
		...
		strFrame+='        <td colspan=3 align=right Author=meizz><span onclick=parent.clearDate() style="font-size:12px;cursor: hand"><u>Clear</u></span>&nbsp;&nbsp;<span onclick=parent.closeLayer() style="font-size:12px;cursor: hand"';
//		strFrame+='        <td colspan=3 align=right Author=meizz><span onclick=parent.closeLayer() style="font-size:12px;cursor: hand"';
		...
		//==================================================== 追加部分 ======================================================
		function createReg(pattern){
			if(outPattern=="YYYYMMDD") return /^(\d{4})(\d{2})(\d{2})$/; 
			if(outPattern=="YYYY-MM-DD") return /^(\d{4})-(\d{2})-(\d{2})$/; 
			if(outPattern=="YYYY/MM/DD") return /^(\d{4})\/(\d{2})\/(\d{2})$/; 
			if(outPattern=="YYYYMM") return /^(\d{4})(\d{2})$/; 
			if(outPattern=="YYYY-MM") return /^(\d{4})-(\d{2})$/; 
			if(outPattern=="YYYY/MM") return /^(\d{4})\/(\d{2})$/; 

			if(outPattern=="YYMMDD") return /^(\d{2})(\d{2})(\d{2})$/; 
			if(outPattern=="YY-MM-DD") return /^(\d{2})-(\d{2})-(\d{2})$/; 
			if(outPattern=="YY/MM/DD") return /^(\d{2})\/(\d{2})\/(\d{2})$/; 
			return /^(\d{4})(\d{2})(\d{2})$/; 
		}
		//==================================================== WEB 页面显示部分 ======================================================
		...
		var outPattern;
		function setday(tt,obj,pattern,callbackfun) //主调函数
		{
			fun_callback = callbackfun;	// 修改时间 2011-12-28 16:19:00 周权
			//if (arguments.length >  3){alert("对不起！传入本控件的参数太多！");return;}	// 修改时间 2011-12-28 16:19:00 周权
			//if (arguments.length != 3){alert("对不起！您没有传回本控件任何参数！");return;} // 修改时间 2011-12-28 16:19:00 周权
			...
			outObject = (obj == null) ? th : obj;
			outButton = (obj == null) ? null : th;	//设定外部点击的按钮
			
			//add by liudeyuan 20081021
			_pos_top=event.y-event.offsetY+18;
			_pos_left=event.x-event.offsetX;
			_Box_Object=_getBoxObj(outObject);

			if ((_pos_left+_OCX_WIDTH)>_Box_Object.clientWidth) _pos_left=_pos_left-_OCX_WIDTH;
			if ((_pos_top+_OCX_HEIGHT)>_Box_Object.clientHeight) _pos_top=_pos_top-_OCX_HEIGHT-18;
			...
			if(r!=null){
				...
				var d= new Date(r[1], r[2],temp_date); 
				if(d.getFullYear()==r[1] && d.getMonth()==r[2] && d.getDate()==temp_date){
					outDate=d;		//保存外部传入的日期
				}
				else outDate="";
		...
		for (var i = n - 100; i < n + 26; i++) //modify by jiangzhe at 20120305
		{
			if (i == m)
		...
		function closeLayer()               //这个层的关闭
		{
			...
			//============== 修改时间 2011-12-28 16:19:00 周权 begin =====================
			if ( fun_callback ) {
				fun_callback();
			}
			//============== 修改时间 2011-12-28 16:19:00 周权 end =======================
		}
		function clearDate()			//Clear Selected Input Date
		{
			outDate="";
			outObject.value="";
			this.closeLayer();
		...
		function meizzToday()  //Today Button
		{
			...
			if(outObject){
				outObject.value=createOutString(meizzTheYear,meizzTheMonth , today);
			}
			closeLayer();
		}
		...
		if (outObject)
		{
			...
			outObject.value= createOutString(yy,mm,n) ; //注：在这里你可以输出改成你想要的格式
			closeLayer(); 
		}
		else {closeLayer(); alert("您所要输出的控件对象并不存在！");}
		}
		
		function createOutString(year,month,date){
			...
			var dd=String(date);
			var outStr=yy +mm + dd;
			var y=(yy).substring(2);

			if(outPattern=="YYYY-MM-DD") outStr=yy + "-" + mm + "-" + dd ; 
			if(outPattern=="YYYY/MM/DD") outStr=yy + "/" + mm + "/" + dd ; 
			if(outPattern=="YYYYMM") outStr=yy +  mm  ; 
			if(outPattern=="YYYY-MM") outStr=yy + "-" + mm ; 
			if(outPattern=="YYYY/MM") outStr=yy + "/" + mm  ; 
			
			if(outPattern=="YYMMDD") outStr=y +  mm +  dd ; 
			if(outPattern=="YY-MM-DD") outStr=y + "-" + mm + "-" + dd ; 
			if(outPattern=="YY/MM/DD") outStr=y + "/" + mm + "/" + dd ; 
			return outStr;
		}
//add by liudeyuan 20081022		
function _getBoxObj(obj){
	var rtObj;
	rtObj=obj;
	if(rtObj.tagName.toLowerCase()=="body"){
		return rtObj;
	}else{
		rtObj=rtObj.parentNode;
		rtObj=_getBoxObj(rtObj);
	}
	return rtObj;
}