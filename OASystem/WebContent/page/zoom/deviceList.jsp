...
function btn_sure()
{
	...
	for(var i=0; i<subBox.length; i++){
		retArray[0] = retArray[0] + $(subBox[i]).parent().find('input[name="dailydeviceid"]').val() + ',';
		retArray[1] = retArray[1] + $(subBox[i]).parent().find('input[name="dailydevicename"]').val() + ',';
	}
	  ...
	  <table class="tb_search check_specialchar">
	    <tbody>
			<tr>
				...
				<td class="td_value" >
					<input id="dailydevicename"  name="dailydevicename" class="input_txt" maxlength="20" value="${dailydevicename}"/>
				</td>
			</tr>	 
			<tr>
		    	...
		    	<c:forEach items="${list}" var="iterator">
		    		<tr class="pg_result_content">
		    			<td align="center" nowrap>
		    				...
		    				<input name="dailydevicename" type="hidden"  value="${iterator.dailydevicename}"/>
		    			</td>
		    			<% num++;%>
