...
function btn_agree() {	
	if ( !window.confirm( Message.getString("MSG_IC_COMM_0015"))) 
		return;
	c_ShowProgressBar(); 
	$("#updateForm").attr( "action", "pd001003agree.do");	
	$("#updateForm").submit();	
}
/*
*名       称: btn_reject()
...
function btn_reject() {	
	if ( !window.confirm( Message.getString("MSG_IC_COMM_0016"))) 
		return;
	c_ShowProgressBar(); 
	$("#updateForm").attr( "action", "pd001003reject.do");	
	$("#updateForm").submit();	
}
/*
*名       称: btn_back()
...
function btn_back() {	
	c_ShowProgressBar(); 
	$("#updateForm").attr( "action", "pd001003back.do");	
	$("#updateForm").submit();
}
</script>
</head>
...
<form id="updateForm" action="" method="post" >
	...
	<%/*共通隐藏字段 end*/%>
	<div class="div_navi"><span><img src="<%= request.getContextPath()%>/resources/images/home.png">&nbsp;您当前的位置：审核管理&nbsp;&gt;&nbsp;部门经理审核&nbsp;&gt;&nbsp;休假申请审核</span></div>
