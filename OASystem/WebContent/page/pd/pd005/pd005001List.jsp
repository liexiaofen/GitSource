...
function btn_search() {
	c_ShowProgressBar(); 
	$("#queryForm").attr( "action", "pd005001search.do");	
	$("#queryForm").submit();
}
/*
*名       称: link_view( id)
...
function link_view( obj){
	...
	c_ShowProgressBar(); 
	$('#viewForm').attr( "action", "pd005001view.do");	
...
function btn_file(){
	...
	c_ShowProgressBar(); 
	$("#fileForm").attr( "action", "pd005001file.do");	
	$("#fileForm").submit();
}
/*
...
<form id="queryForm" action="" method="post" > 
	<div class="div_navi"><span><img src="<%= request.getContextPath()%>/resources/images/home.png">&nbsp;您当前的位置：审核管理&nbsp;&gt;&nbsp;人事归档&nbsp;&gt;&nbsp;人事归档一览</span></div>
	  ...
	  <table class="tb_search check_specialchar">
	    <tbody>
			<tr>
				...
				<td class="td_value" >
					<dict:select id="status" name="status" value="${searchCommand.status}" busiDictTypeId="OA_PD005_Status" cssClass="input_select" nullLabel="全部"></dict:select>
				</td>
			</tr>	 
			<tr>
...
<c:if test="${not empty list}">
	...
	<div class="div_result" id="div_result">  
		<table id="tresult" class="pg_result">
		    <tr class="pg_result_head">
		    	<td width="3%"><c:if test="${ searchCommand.status ne 6}"><input type="checkbox" name="checkAll" id="checkAll" value='abc'/></c:if></td>
				<td width="3%">&nbsp;序号&nbsp;</td>
	...
	<div class="div_result_button">
		<table>
			<tr>
				<td>
					<input name="check" id="check" type="button" class="btn" onClick="btn_file()" onMouseOver="this.className='btn_mouseover'"  onMouseOut="this.className='btn'" value="归&nbsp;档" />
				</td>
			</tr>
		</table>
	</div>
</c:if>
<form action="pd005001page.do" id="pageform" name="pageform" method="post" >
	<%/*共通隐藏字段 start*/%>
	...
	<c:if test="${page.totalRecord gt 0}">
		<div class="div_page"> 
			<script type="text/javascript">
				...
				pagination("page", "pd005001page.do","pageform",curPage,totalPage,firstPage,lastPage);
			</script>
		</div>
	</c:if>
	<!--End Page Infor-->
...
<%-- 详情表单开始  --%>
<form action="pd005001view.do" id="viewForm" name="viewForm" method="post">
	<%/*共通隐藏字段 start*/%>
	<input name="empid" type="hidden"  value="${searchCommand.empid}" />
	<input name="applyno" type="hidden"  value="${searchCommand.applyno}" /> 
	<input name="applytype" type="hidden"  value="${searchCommand.applytype}" />
...
<%-- 详情表单结束  --%>
<form action="pd005001file.do" id="fileForm" name="fileForm" method="post">
	<%/*共通隐藏字段 start*/%>
	<input name="empid" type="hidden"  value="${searchCommand.empid}" />
	<input name="applyno" type="hidden"  value="${searchCommand.applyno}" /> 
	<input name="applytype" type="hidden"  value="${searchCommand.applytype}" />
