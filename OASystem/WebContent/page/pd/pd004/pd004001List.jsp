...
function btn_search() {
	c_ShowProgressBar(); 
	$("#queryForm").attr( "action", "pd004001search.do");	
	$("#queryForm").submit();
}
/*
*名       称: link_view( id)
...
function link_view( obj){
	...
	c_ShowProgressBar(); 
	$('#viewForm').attr( "action", "pd004001view.do");	
...
function btn_check(){
	...
	$("#viewForm").find('input[name="applytype_result"]').val(applytype_result);
	c_ShowProgressBar(); 
	$('#viewForm').attr( "action", "pd004001check.do");	
	$('form[name="viewForm"]').submit();
}
/*
*名       称: btn_reset()
...
<form id="queryForm" action="" method="post" > 
	<div class="div_navi"><span><img src="<%= request.getContextPath()%>/resources/images/home.png">&nbsp;您当前的位置：审核管理&nbsp;&gt;&nbsp;总经理审核&nbsp;&gt;&nbsp;总经理审核一览</span></div>
	  ...
	  <table class="tb_search check_specialchar">
	    <tbody>
			<tr>
				...
				<td class="td_value" >
					<dict:select id="status" name="status" value="${searchCommand.status}" busiDictTypeId="OA_PD004_Status" cssClass="input_select" nullLabel="全部"></dict:select>
				</td>
			</tr>	 
			<tr>
...
<form action="pd004001page.do" id="pageform" name="pageform" method="post" >
	<%/*共通隐藏字段 start*/%>
	...
	<c:if test="${page.totalRecord gt 0}">
		<div class="div_page"> 
			<script type="text/javascript">
				...
				pagination("page", "pd004001page.do","pageform",curPage,totalPage,firstPage,lastPage);
			</script>
		</div>
	</c:if>
	<!--End Page Infor-->
...
<%-- 详情表单开始  --%>
<form action="pd004001view.do" id="viewForm" name="viewForm" method="post">
	<%/*共通隐藏字段 start*/%>
	<input name="empid" type="hidden"  value="${searchCommand.empid}" />
	<input name="applyno" type="hidden"  value="${searchCommand.applyno}" /> 
	<input name="applytype" type="hidden"  value="${searchCommand.applytype}" />
